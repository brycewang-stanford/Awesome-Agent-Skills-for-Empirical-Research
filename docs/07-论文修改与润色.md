...
### humanize-chinese（中文学术 AI 检测 + 改写 CLI）

| 属性 | 说明 |
|------|------|
| **来源** | [voidborne-d/humanize-chinese](https://github.com/voidborne-d/humanize-chinese) |
| **功能** | 中文 AI 文本检测与改写工具：17 类检测特征 + 7 风格改写器（academic / novel / blog / casual / 小红书 / 微信 / 文学），针对知网 / 万方 / 维普 / Turnitin 中文版实测调参 |
| **形态** | 独立 Python CLI + 库，0 第三方依赖。可在非 Claude 环境直接调用，也可在 Claude Code / OpenClaw / Hermes 等 agent 内当工具调用 |
| **安装** | `pip install humanize-chinese` 或 `git clone https://github.com/voidborne-d/humanize-chinese && pip install -e .` |

**与 chinese-de-aigc / humanizer_academic 的差异**：

| 维度 | chinese-de-aigc | humanizer_academic | humanize-chinese |
|------|------------------|--------------------|--------------------|
| 形态 | Markdown SKILL.md（agent 内） | Markdown SKILL.md（agent 内） | Python CLI + 库（pipeline 可调用） |
...
| 长篇支持 | 通用 | 通用 | 长度 ≥1500 字自动切到长篇 LR；段落级特征（段内句长 CV、段长 CV、跨段 3-gram 重复率） |

**核心特色（v5 段落级特化）**：

- **3 个段落级信号**：段内句长 CV（calibration d=−2.08）、段长 CV（d=−1.49）、跨段 3-gram 重复率（d=+1.13）。LR holdout 0.897 → 0.926，AI 长篇 vs 人类 LR gap 58 → 87.8（+50%）
- **改写端 5 步段落级反制**：合并段内相邻短-中句、长段后插独立短反射段、跨段重复 2-char 词替换为 cilin 同义词、CV-aware 守卫、academic / markdown 注入 formal pool（"客观地说" / "诚然如此。"）而非 casual filler（"颇有道理。"）
- **HC3 100 短问答基准**：95% 正确分离，平均降幅 -63（短样本）/ -55（长篇）
- **可复现**：默认 `--best-of-n 10 --seed 42`，0 grammar defects，段落保留率 100%
- **场景感知 detector**：`--scene academic / novel / blog / news / auto`，长度 ≥1500 字自动切到长篇 LR 校准

**使用建议**：

- 中文论文 batch 处理（不在 Claude Code 内）→ humanize-chinese CLI
- agent 内交互式改写 → chinese-de-aigc Skill
- 中英双语论文 → humanize-chinese（中文段）+ humanizer_academic（英文段）
- 需要程序化得分 + 自动重试 → humanize-chinese（best-of-n 选最低 AIGC 分）
