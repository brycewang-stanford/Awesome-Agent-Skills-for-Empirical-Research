...
| [voidborne-d/humanize-chinese](https://github.com/voidborne-d/humanize-chinese) 🇨🇳 | **中文学术独立 CLI 工具（不是 Skill 形态，与 chinese-de-aigc 互补）**。Python 0 依赖，17 类中文 AI 模式检测 + 7 风格改写器（academic / novel / blog / casual / 小红书 / 微信 / 文学），针对知网/万方/维普实测调参。v5 引入段落级特征（段内句长 CV、段长 CV、跨段 3-gram 重复率），长篇 LR holdout 0.897 → 0.926；学术模式注入 formal pool（"客观地说"）而非 casual filler；HC3 100 短问答基准 95% 正确分离，长篇平均降幅 -55 / 短样本 -63。`pip install` 即可，可独立调用或串入任意 pipeline | 中文学位论文 / 期刊投稿 / 长篇博客 / 需要在非 Claude 环境跑批降 AIGC 的场景 | （社区 PR 贡献） |
